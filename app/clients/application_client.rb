# ❤️ https://github.com/jumpstart-pro/jumpstart-pro-rails/blob/main/app/clients/application_client.rb

require "ostruct" # not included by default
...
class ApplicationClient
  ...
  RATE_LIMIT_HEADER = "x_ratelimit_reset"
  RATE_LIMIT_FALLBACK_DURATION = 10.minutes
  ...
  # Returns the RATE_LIMIT_HEADER from the current class
  def rate_limit_header = self.class::RATE_LIMIT_HEADER

  # Returns the RATE_LIMIT_FALLBACK_DURATION from the current class, this is used
  # if headers don't contain a defined time to reset
  def rate_limit_fallback_duration = self.class::RATE_LIMIT_FALLBACK_DURATION
  ...
  def handle_response(response)
    ...
    when "420", "429"
      raise RateLimit.new(response.body, response.headers[rate_limit_header&.to_sym] || rate_limit_fallback_duration&.from_now&.to_i)
    when "500"
